...
function daysConverter(num) {
    if (num >= 0) {
        ...
        var days = num % 7;
        return "".concat(weeks, " weeks and ").concat(days, " days");
    }
    else {
...
function clothesSuggestor(temp) {
    if (temp <= 10) {
...
function taxCalculator(unitsConsumed) {
    var unitPrice = 12;
    var totalBill = unitPrice * unitsConsumed;
    if (unitsConsumed < 100) {
        return "Added tax is 0";
    }
    else if (unitsConsumed >= 100 && unitsConsumed < 200) {
        return "Added tax is 10% = ".concat(totalBill * 0.10);
    }
    else if (unitsConsumed >= 200 && unitsConsumed < 300) {
        return "Added tax is 15% = ".concat(totalBill * 0.15);
    }
    else if (unitsConsumed >= 300 && unitsConsumed < 400) {
        return "Added tax is 18% = ".concat(totalBill * 0.18);
    }
    else if (unitsConsumed >= 400 && unitsConsumed <= 500) {
        return "Added tax is 20% = ".concat(totalBill * 0.20);
    }
    else {
        return "Added tax is 25% = ".concat(totalBill * 0.25);
    }
}
